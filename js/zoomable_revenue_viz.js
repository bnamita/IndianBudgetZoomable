function ZoomableRevenueViz() {
}
ZoomableRevenueViz.prototype = {
    createViz: function(dataFile){
        var self = this;
        var width = 940;
        this.totalSize = 0;
        height = 860,
        ...
        d3.json(dataFile, function(error, root) {
            ...
            path.enter().append("path")
                ...
                .style("opacity", 0.85)
                .style("fill", function(d) { return self.colors(d); })
            ...
            function click(d) {
                ...
                if (d.depth == 0) {
                    ...
                    d3.select("#centre-label").text("Total Receipts")
            ...
            function displayDetails (d){
                ...
                d3.select("#segment").text(d.name)//.style("color", self.colors(d));;
                ...
                d3.select("#percent").text(d.value != 0 ? "( " + percentage + "% " : " ")//.style("color", self.colors(d));
            }
    ...
    colors: function (d) {
        if (d.name == "Tax Revenue") return "#D81B1F";
        if (d.name == "Non-Tax Revenue") return "#0064A2";
        if (d.name == "Capital Receipts") return "#49AB57";

        if (d.parent && d.parent.name == "Tax Revenue") return "#E95B2E";
        if (d.parent && d.parent.name == "Non-Tax Revenue") return "#488FC2";
        if (d.parent && d.parent.name == "Capital Receipts") return "#76B847";

        if (d.parent && d.parent.parent.name == "Tax Revenue") return "#fe9929";
        if (d.parent && d.parent.parent.name == "Non-Tax Revenue") return "#bdc9e1";
